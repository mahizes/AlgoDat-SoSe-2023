...
public class EdgeWeightedGraph {
    ...
    public void setCoordinates(double [][]coord) {
        if (coord.length == 0) {
            this.coord = new double[this.V][];
            for (int i = 0; i < this.V; i++) {
                this.coord[i] = new double[2];
            }
        } else if (coord.length != this.V) {
            throw new IllegalArgumentException("The number of coordinate sets must match the number of vertices.");
        } else {
            this.coord = coord;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 5; i++) {
            coordinates[i][0] = Math.random() * 100;  // x-coordinate
            coordinates[i][1] = Math.random() * 100;  // y-coordinate
        }
        graph.setCoordinates(coordinates);

        // Print out the coordinates
        double[][] graphCoordinates = graph.getCoordinates();
        for (int i = 0; i < graphCoordinates.length; i++) {
            System.out.println("Vertex " + i + ": (" + graphCoordinates[i][0] + ", " + graphCoordinates[i][1] + ")");
        }
    }
