...
public class ContentAwareImageResizing {
    ...
    public int[] leastContrastImageVPath() {
        ...
        int s = sx * sy; // source node
        int t = s + 1; // target node
        ShortestPathsTopological sp = new ShortestPathsTopological(G, s);

        // Extract the path from the source to the target
        Stack<Integer> pathStack = sp.pathTo(t);
        int[] path = new int[sy];
        for (int y = 0; y < sy; y++) {
            path[y] = nodeToCoordinate(pathStack.pop()).x;
        }
        return path;
    }
